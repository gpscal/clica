# Clino Extension Architecture & Development Guide

## Project Overview

Clino is a VSCode extension that provides AI assistance through a combination of a core extension backend and a React-based webview frontend. The extension is built with TypeScript and follows a modular architecture pattern.
...
The WebviewProvider maintains a reference to the Controller and delegates message handling to it. It also handles the creation of both sidebar and tab panel webviews, allowing Clino to be used in different contexts within VSCode.
...
Clino supports multiple AI providers through a modular API provider system. Each provider is implemented as a separate module in the `src/api/providers/` directory and follows a common interface.
...
Clino supports separate model configurations for Plan and Act modes:
...
class Task {
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    const previousRequest = this.clinoMessages[previousApiReqIndex]
...
class Task {
  async resumeTaskFromHistory() {
    // 1. Load saved state
    this.clinoMessages = await getSavedClinoMessages(this.getContext(), this.taskId)
  ...
  private async saveTaskState() {
    ...
    await saveClinoMessages(this.getContext(), this.taskId, this.clinoMessages)
...
Clino implements a dual-mode system that separates planning from execution:
...
Clino supports two types of MCP server connections:
...
This guide provides a comprehensive overview of the Clino extension architecture, with special focus on state management, data persistence, and code organization. Following these patterns ensures robust feature implementation with proper state handling across the extension's components.
...
Contributions to the Clino extension are welcome! Please follow these guidelines:
...
The `.clineignore` file allows users to specify files and directories that Clino should not access. When implementing new features, respect the `.clineignore` rules and ensure that your code does not attempt to read or modify ignored files.
