...
func main() {
	rootCmd := &cobra.Command{
		Use:   "clica [prompt]",
		Short: "Clica CLI - AI-powered coding assistant",
		Long: `A command-line interface for interacting with Clica AI coding assistant.

Start a new task by providing a prompt:
  clica "Create a new Python script that prints hello world"

Or pipe a prompt via stdin:
  echo "Create a todo app" | clica
  cat prompt.txt | clica --yolo
...
see the manual page: man clica`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !cmd.Flags().Changed("address") {
				...
				if !isUserReadyToUse(ctx, instanceAddress) {
					...
					if !isUserReadyToUse(ctx, instanceAddress) {
						return fmt.Errorf("credentials still not configured - please run 'clica auth' to complete setup")
					}

					fmt.Printf("\n%s\n\n", renderer.Dim("✓ Setup complete, you can now use the Clica CLI"))
				}
			} else {
	...
	rootCmd.PersistentFlags().StringVar(&coreAddress, "address", fmt.Sprintf("localhost:%d", common.DEFAULT_CLICA_CORE_PORT), "Clica Core gRPC address")
...
func promptForInitialTask(ctx context.Context, instanceAddress, modeFlag string) (string, error) {
	...
	form := huh.NewForm(
		huh.NewGroup(
			huh.NewText().
				Title("Start a new Clica task").
				Description("What would you like Clica to help you with?").
...
func showSessionBanner(ctx context.Context, instanceAddress, modeFlag string) {
	...
	// Get current working directory (this is what Clica will use)
