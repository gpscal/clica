...
func main() {
	rootCmd := &cobra.Command{
		Use:   "clino [prompt]",
		Short: "Clino CLI - AI-powered coding assistant",
		Long: `A command-line interface for interacting with Clino AI coding assistant.

Start a new task by providing a prompt:
  clino "Create a new Python script that prints hello world"

Or pipe a prompt via stdin:
  echo "Create a todo app" | clino
  cat prompt.txt | clino --yolo
...
see the manual page: man clino`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !cmd.Flags().Changed("address") {
				...
				if !isUserReadyToUse(ctx, instanceAddress) {
					...
					if !isUserReadyToUse(ctx, instanceAddress) {
						return fmt.Errorf("credentials still not configured - please run 'clino auth' to complete setup")
					}

					fmt.Printf("\n%s\n\n", renderer.Dim("✓ Setup complete, you can now use the Clino CLI"))
				}
			} else {
	...
	rootCmd.PersistentFlags().StringVar(&coreAddress, "address", fmt.Sprintf("localhost:%d", common.DEFAULT_CLINO_CORE_PORT), "Clino Core gRPC address")
...
func promptForInitialTask(ctx context.Context, instanceAddress, modeFlag string) (string, error) {
	...
	form := huh.NewForm(
		huh.NewGroup(
			huh.NewText().
				Title("Start a new Clino task").
				Description("What would you like Clino to help you with?").
...
func showSessionBanner(ctx context.Context, instanceAddress, modeFlag string) {
	...
	// Get current working directory (this is what Clino will use)
