...
// Clica provider specific code

func HandleClineAuth(ctx context.Context) error {
	verboseLog("Authenticating with Clica...")
	...
	// Configure default Clica model after successful authentication
	if err := configureDefaultClineModel(ctx); err != nil {
		fmt.Printf("Warning: Could not configure default Clica model: %v\n", err)
		fmt.Println("You can configure a model later with 'clica auth' and selecting 'Change Clica model'")
	}
...
func signOut(ctx context.Context) error {
	...
	if _, err = client.Account.AccountLogoutClicked(ctx, &clica.EmptyRequest{}); err != nil {
		return err
	}

	isSessionAuthenticated = false
	fmt.Println("You have been signed out of Clica.")
	return nil
}
...
func signOutDialog(ctx context.Context) error {
	...
	form := huh.NewForm(
		huh.NewGroup(
			huh.NewConfirm().
				Title("You are already signed in to Clica.").
...
func signIn(ctx context.Context) error {
	...
	// Initiate login (opens browser with callback URL from clica-core's AuthHandler)
	...
	response, err := client.Account.AccountLoginClicked(ctx, &clica.EmptyRequest{})
	if err != nil {
	...
	if err := listener.WaitForAuthentication(5 * time.Minute); err != nil {
		...
		fmt.Println("  Please try again with 'clica auth'")
		return err
	}
...
func IsAuthenticated(ctx context.Context) bool {
	...
	_, err = client.Account.GetUserCredits(ctx, &clica.EmptyRequest{})
	if err == nil {
...
// HandleChangeClineModel allows Clica-authenticated users to change their Clica model selection. Hidden when not authenticated.
func HandleChangeClineModel(ctx context.Context) error {
	// Ensure user is authenticated
	if !IsAuthenticated(ctx) {
		return fmt.Errorf("you must be authenticated with Clica to change models. Run 'clica auth' to sign in")
	}
	...
	// Launch Clica model selection
	return SelectClineModel(ctx, manager)
}

// configureDefaultClineModel configures the default Clica model after authentication
func configureDefaultClineModel(ctx context.Context) error {
	verboseLog("Configuring default Clica model...")
	...
	// Set default Clica model
	return SetDefaultClineModel(ctx, manager)
}

// HandleSelectOrganization allows Clica-authenticated users to select which organization to use
func HandleSelectOrganization(ctx context.Context) error {
	// Ensure user is authenticated
	if !IsAuthenticated(ctx) {
		return fmt.Errorf("you must be authenticated with Clica to select an organization. Run 'clica auth' to sign in")
	}
	...
	orgsResponse, err := client.Account.GetUserOrganizations(ctx, &clica.EmptyRequest{})
	if err != nil {
	...
	req := &clica.UserOrganizationUpdateRequest{
