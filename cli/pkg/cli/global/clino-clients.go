...
// ClinoClients manages Clino instances using the new registry system
type ClinoClients struct {
	registry *ClientRegistry
}

// NewClinoClients creates a new ClinoClients instance
func NewClinoClients(configPath string) *ClinoClients {
...
func (c *ClinoClients) Initialize(ctx context.Context) error {
...
// StartNewInstance starts a new Clino instance and waits for clino-core to self-register
func (c *ClinoClients) StartNewInstance(ctx context.Context) (*common.CoreInstanceInfo, error) {
	...
	if Config.Verbose {
		fmt.Printf("Starting new Clino instance on ports %d (core) and %d (host bridge)\n", corePort, hostPort)
	}

	// Start clino-host first
	hostCmd, err := startClineHost(hostPort, corePort)
	if err != nil {
		return nil, fmt.Errorf("failed to start clino-host: %w", err)
	}

	// Start clino-core (it will register itself in SQLite locks database)
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to start clino-core: %w", err)
	}
...
// StartNewInstanceAtPort starts a new Clino instance at the specified port and waits for self-registration
func (c *ClinoClients) StartNewInstanceAtPort(ctx context.Context, corePort int) (*common.CoreInstanceInfo, error) {
	...
	if common.IsInstanceHealthy(ctx, coreAddress) {
		return nil, fmt.Errorf("port %d is already in use by another Clino instance", corePort)
	}
...
func (c *ClinoClients) GetRegistry() *ClientRegistry {
	return c.registry
}

// EnsureInstanceAtAddress ensures an instance exists at the given address, starting one if needed
func (c *ClinoClients) EnsureInstanceAtAddress(ctx context.Context, address string) error {
	// Expect host:port everywhere
	normalized := address
	if normalized == "" {
		normalized = fmt.Sprintf("localhost:%d", common.DEFAULT_CLINO_CORE_PORT)
	}
...
func startClineHost(hostPort, corePort int) (*exec.Cmd, error) {
	if Config.Verbose {
		fmt.Printf("Starting clino-host on port %d\n", hostPort)
	}

	// Get the directory where the clino binary is located
	execPath, err := os.Executable()
	if err != nil {
		return nil, fmt.Errorf("failed to get executable path: %w", err)
	}
	binDir := path.Dir(execPath)
	clineHostPath := path.Join(binDir, "clino-host")

	// Start the clino-host process
	cmd := exec.Command(clineHostPath,
		"--verbose",
		"--port", fmt.Sprintf("%d", hostPort))

	// Create logs directory in ~/.clino/logs
	...
	logFileName := fmt.Sprintf("clino-host-%s-localhost-%d.log", timestamp, hostPort)
	...
	if Config.Verbose {
		fmt.Printf("Started clino-host (PID: %d)\n", cmd.Process.Pid)
		fmt.Printf("Logging clino-host output to: %s\n", logFilePath)
	}
	return cmd, nil
}

// KillInstanceByAddress kills a Clino instance by its address
...
func startClineCore(corePort, hostPort int) (*exec.Cmd, error) {
	if Config.Verbose {
		fmt.Printf("Starting clino-core on port %d (with hostbridge on %d)\n", corePort, hostPort)
	}
	...
	// /opt/homebrew/bin/clino -> /opt/homebrew/lib/node_modules/clino/bin/clino
	...
	clineCorePath := path.Join(installDir, "clino-core.js")
	...
	if Config.Verbose {
		...
		fmt.Printf("Looking for clino-core.js at: %s\n", clineCorePath)
	}

	// Check if clino-core.js exists at the primary location
	var finalClineCorePath string
	var finalInstallDir string
	if _, err := os.Stat(clineCorePath); os.IsNotExist(err) {
		// Development mode: Try ../../dist-standalone/clino-core.js
		// This handles the case where we're running from cli/bin/clino
		devClineCorePath := path.Join(binDir, "..", "..", "dist-standalone", "clino-core.js")
		...
		if _, err := os.Stat(devClineCorePath); os.IsNotExist(err) {
			return nil, fmt.Errorf("clino-core.js not found at '%s' or '%s'. Please ensure you're running from the correct location or reinstall with 'npm install -g clino'", clineCorePath, devClineCorePath)
		}
		
		finalClineCorePath = devClineCorePath
		finalInstallDir = devInstallDir
		if Config.Verbose {
			fmt.Printf("Using development mode: clino-core.js found at %s\n", finalClineCorePath)
		}
	} else {
		finalClineCorePath = clineCorePath
		finalInstallDir = installDir
		if Config.Verbose {
			fmt.Printf("Using production mode: clino-core.js found at %s\n", finalClineCorePath)
		}
	}
	...
	// Create timestamped log file
	timestamp := time.Now().Format("2006-01-02-15-04-05")
	logFileName := fmt.Sprintf("clino-core-%s-localhost-%d.log", timestamp, corePort)
	logFilePath := path.Join(logsDir, logFileName)
	logFile, err := os.Create(logFilePath)
	if err != nil {
		return nil, fmt.Errorf("failed to create log file: %w", err)
	...
	// Start the clino-core process with --config flag using system node
	...
	if Config.Verbose {
		fmt.Printf("Started clino-core (PID: %d)\n", cmd.Process.Pid)
		fmt.Printf("Logging clino-core output to: %s\n", logFilePath)
	}
	return cmd, nil
}
