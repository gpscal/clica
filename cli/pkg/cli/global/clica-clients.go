...
// ClicaClients manages Clica instances using the new registry system
type ClicaClients struct {
	registry *ClientRegistry
}

// NewClicaClients creates a new ClicaClients instance
func NewClicaClients(configPath string) *ClicaClients {
...
func (c *ClicaClients) Initialize(ctx context.Context) error {
...
// StartNewInstance starts a new Clica instance and waits for clica-core to self-register
func (c *ClicaClients) StartNewInstance(ctx context.Context) (*common.CoreInstanceInfo, error) {
	...
	if Config.Verbose {
		fmt.Printf("Starting new Clica instance on ports %d (core) and %d (host bridge)\n", corePort, hostPort)
	}

	// Start clica-host first
	hostCmd, err := startClineHost(hostPort, corePort)
	if err != nil {
		return nil, fmt.Errorf("failed to start clica-host: %w", err)
	}

	// Start clica-core (it will register itself in SQLite locks database)
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to start clica-core: %w", err)
	}
...
// StartNewInstanceAtPort starts a new Clica instance at the specified port and waits for self-registration
func (c *ClicaClients) StartNewInstanceAtPort(ctx context.Context, corePort int) (*common.CoreInstanceInfo, error) {
	...
	if common.IsInstanceHealthy(ctx, coreAddress) {
		return nil, fmt.Errorf("port %d is already in use by another Clica instance", corePort)
	}
...
func (c *ClicaClients) GetRegistry() *ClientRegistry {
	return c.registry
}

// EnsureInstanceAtAddress ensures an instance exists at the given address, starting one if needed
func (c *ClicaClients) EnsureInstanceAtAddress(ctx context.Context, address string) error {
	// Expect host:port everywhere
	normalized := address
	if normalized == "" {
		normalized = fmt.Sprintf("localhost:%d", common.DEFAULT_CLICA_CORE_PORT)
	}
...
func startClineHost(hostPort, corePort int) (*exec.Cmd, error) {
	if Config.Verbose {
		fmt.Printf("Starting clica-host on port %d\n", hostPort)
	}

	// Get the directory where the clica binary is located
	execPath, err := os.Executable()
	if err != nil {
		return nil, fmt.Errorf("failed to get executable path: %w", err)
	}
	binDir := path.Dir(execPath)
	clineHostPath := path.Join(binDir, "clica-host")

	// Start the clica-host process
	cmd := exec.Command(clineHostPath,
		"--verbose",
		"--port", fmt.Sprintf("%d", hostPort))

	// Create logs directory in ~/.clica/logs
	...
	logFileName := fmt.Sprintf("clica-host-%s-localhost-%d.log", timestamp, hostPort)
	...
	if Config.Verbose {
		fmt.Printf("Started clica-host (PID: %d)\n", cmd.Process.Pid)
		fmt.Printf("Logging clica-host output to: %s\n", logFilePath)
	}
	return cmd, nil
}

// KillInstanceByAddress kills a Clica instance by its address
...
func startClineCore(corePort, hostPort int) (*exec.Cmd, error) {
	if Config.Verbose {
		fmt.Printf("Starting clica-core on port %d (with hostbridge on %d)\n", corePort, hostPort)
	}
	...
	// /opt/homebrew/bin/clica -> /opt/homebrew/lib/node_modules/clica/bin/clica
	...
	clineCorePath := path.Join(installDir, "clica-core.js")
	...
	if Config.Verbose {
		...
		fmt.Printf("Looking for clica-core.js at: %s\n", clineCorePath)
	}

	// Check if clica-core.js exists at the primary location
	var finalClineCorePath string
	var finalInstallDir string
	if _, err := os.Stat(clineCorePath); os.IsNotExist(err) {
		// Development mode: Try ../../dist-standalone/clica-core.js
		// This handles the case where we're running from cli/bin/clica
		devClineCorePath := path.Join(binDir, "..", "..", "dist-standalone", "clica-core.js")
		...
		if _, err := os.Stat(devClineCorePath); os.IsNotExist(err) {
			return nil, fmt.Errorf("clica-core.js not found at '%s' or '%s'. Please ensure you're running from the correct location or reinstall with 'npm install -g clica'", clineCorePath, devClineCorePath)
		}
		
		finalClineCorePath = devClineCorePath
		finalInstallDir = devInstallDir
		if Config.Verbose {
			fmt.Printf("Using development mode: clica-core.js found at %s\n", finalClineCorePath)
		}
	} else {
		finalClineCorePath = clineCorePath
		finalInstallDir = installDir
		if Config.Verbose {
			fmt.Printf("Using production mode: clica-core.js found at %s\n", finalClineCorePath)
		}
	}
	...
	// Create timestamped log file
	timestamp := time.Now().Format("2006-01-02-15-04-05")
	logFileName := fmt.Sprintf("clica-core-%s-localhost-%d.log", timestamp, corePort)
	logFilePath := path.Join(logsDir, logFileName)
	logFile, err := os.Create(logFilePath)
	if err != nil {
		return nil, fmt.Errorf("failed to create log file: %w", err)
	...
	// Start the clica-core process with --config flag using system node
	...
	if Config.Verbose {
		fmt.Printf("Started clica-core (PID: %d)\n", cmd.Process.Pid)
		fmt.Printf("Logging clica-core output to: %s\n", logFilePath)
	}
	return cmd, nil
}
