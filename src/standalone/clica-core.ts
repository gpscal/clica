...
function setupHostProvider(extensionContext: any, extensionDir: string, dataDir: string) {
	...
	// clica-core expects the binaries to be unpacked in the directory where it is running.
...
/**
 * Gracefully shutdown the clica-core process by:
...
function showHelp() {
	console.log(`
Clica Core - Standalone Server

Usage: node clica-core.js [options]
...
Options:
  ...
  -c, --config <path>            Directory for Clica data storage (default: ~/.clica)
