...
async function main() {
	...
	try {
		...
		// Set the controller on the webview provider (needed for compatibility)
		webviewProvider.controller = webviewProvider.controller || (webviewProvider as any).controller
...
function setupHostProvider(extensionContext: any, extensionDir: string, dataDir: string) {
	...
	// clino-core expects the binaries to be unpacked in the directory where it is running.
...
/**
 * Gracefully shutdown the clino-core process by:
...
function showHelp() {
	console.log(`
Clino Core - Standalone Server

Usage: node clino-core.js [options]
...
Options:
  ...
  -c, --config <path>            Directory for Clino data storage (default: ~/.clino)
