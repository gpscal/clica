...
/**
 * Validates diff content to ensure SEARCH and REPLACE blocks are not identical.
 * This prevents no-op edits where the AI generates identical search and replace content.
 *
 * @param diffContent The diff content to validate
 * @returns Object with ok flag and optional error details
 */
export function validateDiffBlocks(
	diffContent: string,
): { ok: true } | { ok: false; blockNumber: number; searchContent: string; replaceContent: string } {
	...
	for (const line of lines) {
		...
		if (isReplaceBlockEnd(line)) {
			...
			const searchTrimmed = currentSearchContent.trim()
			const replaceTrimmed = currentReplaceContent.trim()

			// Check if search and replace are identical (and not empty)
			if (searchTrimmed && searchTrimmed === replaceTrimmed) {
				return {
					ok: false,
					blockNumber,
