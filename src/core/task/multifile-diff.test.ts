...
describe("multifile-diff", () => {
	...
	describe("showChangedFilesDiff", () => {
		...
		const mockMessages: ClicaMessage[] = [
			{
				ts: mockMessageTs,
				type: "say",
		...
		beforeEach(() => {
			messageStateHandlerStub.getClicaMessages.returns(mockMessages)
		})
		...
		it("should successfully show diff for changes since last task completion", async () => {
			...
			const messagesWithCompletion: ClicaMessage[] = [
				{
					ts: 1234567000,
					type: "say",
			...
			messageStateHandlerStub.getClicaMessages.returns(messagesWithCompletion)
		...
		it("should handle message not found error", async () => {
			// Arrange
			messageStateHandlerStub.getClicaMessages.returns([])
		...
		it("should handle missing checkpoint hash", async () => {
			// Arrange
			const messagesWithoutHash: ClicaMessage[] = [
				{
					ts: mockMessageTs,
					type: "say",
					say: "text",
			...
			messageStateHandlerStub.getClicaMessages.returns(messagesWithoutHash)
		...
		it("should use first checkpoint when no last completion found", async () => {
			// Arrange
			const messagesWithFirstCheckpoint: ClicaMessage[] = [
				{
					ts: 1234567000,
					type: "say",
			...
			messageStateHandlerStub.getClicaMessages.returns(messagesWithFirstCheckpoint)
