...
describe("multifile-diff", () => {
	...
	describe("showChangedFilesDiff", () => {
		...
		const mockMessages: ClinoMessage[] = [
			{
				ts: mockMessageTs,
				type: "say",
		...
		beforeEach(() => {
			messageStateHandlerStub.getClinoMessages.returns(mockMessages)
		})
		...
		it("should successfully show diff for changes since last task completion", async () => {
			...
			const messagesWithCompletion: ClinoMessage[] = [
				{
					ts: 1234567000,
					type: "say",
			...
			messageStateHandlerStub.getClinoMessages.returns(messagesWithCompletion)
		...
		it("should handle message not found error", async () => {
			// Arrange
			messageStateHandlerStub.getClinoMessages.returns([])
		...
		it("should handle missing checkpoint hash", async () => {
			// Arrange
			const messagesWithoutHash: ClinoMessage[] = [
				{
					ts: mockMessageTs,
					type: "say",
					say: "text",
			...
			messageStateHandlerStub.getClinoMessages.returns(messagesWithoutHash)
		...
		it("should use first checkpoint when no last completion found", async () => {
			// Arrange
			const messagesWithFirstCheckpoint: ClinoMessage[] = [
				{
					ts: 1234567000,
					type: "say",
			...
			messageStateHandlerStub.getClinoMessages.returns(messagesWithFirstCheckpoint)
