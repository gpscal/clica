...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`
	...
	identicalBlockError: (blockNumber: number, searchContent: string, relPath: string, originalContent: string | undefined) =>
		`The SEARCH/REPLACE block #${blockNumber} contains identical content in both the SEARCH and REPLACE sections, which would result in no changes to the file. This is not a valid edit operation.\n\n` +
		`The problematic block contains:\n` +
		`<block_content>\n${searchContent.trim()}\n</block_content>\n\n` +
		`To fix this issue:\n` +
		`1. If you want to modify this content, ensure the REPLACE section contains the CHANGED version, not the original.\n` +
		`2. If you don't need to modify this content, remove this SEARCH/REPLACE block entirely.\n` +
		`3. Remember: The SEARCH section should contain the ORIGINAL text from the file, and the REPLACE section should contain what you want to CHANGE IT TO.\n\n` +
		`The file was not modified and remains in its original state:\n\n` +
		`<file_content path="${relPath.toPosix()}">\n${originalContent}\n</file_content>\n\n` +
		`Please retry the replace_in_file operation with proper SEARCH/REPLACE blocks that actually make changes to the file.`,
