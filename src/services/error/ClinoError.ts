...
export class ClinoError extends Error {
	readonly title = "ClinoError"
	...
	/**
	 * Parses a stringified error into a ClinoError instance.
	 */
	static parse(errorStr?: string, modelId?: string): ClinoError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return ClinoError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a ClinoError instance.
	 * Always returns a ClinoError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): ClinoError {
		try {
			// If already a ClinoError, return it directly to prevent infinite recursion
			if (error instanceof ClinoError) {
				return error
			}
			return new ClinoError(JSON.parse(error), modelId, providerId)
		} catch {
			return new ClinoError(error, modelId, providerId)
		}
	}

	public isErrorType(type: ClinoErrorType): boolean {
		return ClinoError.getErrorType(this) === type
	}
	...
	static getErrorType(err: ClinoError): ClinoErrorType | undefined {
		...
		if (message) {
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return ClinoErrorType.RateLimit
			}
		}
