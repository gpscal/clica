	...
	async run(terminal: vscode.Terminal, command: string) {
		...
		if (terminal.shellIntegration && terminal.shellIntegration.executeCommand) {
			...
			for await (let data of stream) {
				...
				// these markers indicate the command is some kind of local dev server recompiling the app, which we want to wait for output of before sending request to clino
