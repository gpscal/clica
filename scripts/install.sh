...
check_existing_installation() {
    ...
    if [ -d "$INSTALL_DIR/bin" ] && [ -f "$INSTALL_DIR/bin/clino" ]; then
        # Extract version from clino binary
        local installed_version=$("$INSTALL_DIR/bin/clino" version 2>/dev/null | head -1 | grep -o 'v[0-9]\+\.[0-9]\+\.[0-9]\+' || echo "")
        ...
        if [ -n "$installed_version" ] && [ "$installed_version" = "$cli_tag_version" ]; then
            echo ""
            print_ok "Clino ${MAGENTA}${BOLD}$installed_version${NC} already installed"
            echo ""
...
install_cline() {
    ...
    echo -e "${MAGENTA}${BOLD}"
    local package_file="$tmp_dir/clino.tar.gz"
    ...
    print_ok "Clino installed to ${MAGENTA}${BOLD}$INSTALL_DIR${NC}"
}
...
verify_installation() {
    ...
    local cline_bin="$INSTALL_DIR/bin/clino"
...
print_success() {
    ...
    echo ""
    print_message "$NC" "Run this to start using ${MAGENTA}${BOLD}clino${NC} immediately:"
    echo ""
...
main() {
    echo ""
    print_box "CLINO IS COOKING" "$MAGENTA$BOLD" 48
    echo ""
