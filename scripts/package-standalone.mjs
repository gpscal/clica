...
function setupFakeNodeModules() {
	...
	if (!fs.existsSync(vscodeSource)) {
		console.warn('Warning: vscode stub not found in node_modules, skipping fake_node_modules creation');
		return;
	}

	if (!fs.existsSync(fakeNodeModulesDir)) {
		console.log('Creating fake_node_modules with vscode stub...');
		fs.mkdirSync(fakeNodeModulesDir, { recursive: true });
		...
		if (fs.existsSync(vscodeSource)) {
			...
			const platform = os.platform();
			if (platform === 'win32') {
				execSync(\`xcopy /E /I /Y "\${vscodeSource}" "\${vscodeDest}"\`, { stdio: 'ignore' });
			} else {
				execSync(\`cp -r "\${vscodeSource}" "\${vscodeDest}"\`, { stdio: 'ignore' });
			}
			console.log('✓ fake_node_modules created');
		}
	}
}
...
function ensureDependencies() {
	...
	try {
		...
		console.warn('Warning: npm install failed, but continuing...');
...
try {
	...
	console.error('Installation failed. Please check the error message above.');
	process.exit(1);
}
`
